...
## What do previously healthy people have risks of becoming ill with?

```{r}

patient_risk_clean %>% 
  filter(if_all(ends_with("in_prior_year"), ~ .x != 1)) %>% 
  select(starts_with("predicted_risk_of_"))
  
```

It turns out there aren't many of these people, and they're not at risk of much ... 

## What risk factors do people expected to get dementia have?

```{r}

patient_risk_clean %>% 
  filter(predicted_risk_of_dementia >= 0.05)
  filter(if_any(starts_with("predicted_risk_of_"), ~ .x >= 0.25)) %>% 
  select(starts_with("predicted_risk_of_"))
